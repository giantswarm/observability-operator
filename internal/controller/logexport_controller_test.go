...
var _ = Describe("LogExport Controller", Ordered, func() {
	...
	It("renders the ConfigMap the HelmRelease reads, and no Secret without credentials", func() {
		...
		By("leaving the object untouched when a reconcile changes nothing")
		// resourceVersion rather than LastTransitionTime, which only moves when the
		// status flips. An unchanged one is what says no watch event follows.
		reconcileTwice(auditExport)
		after := &observabilityv1alpha1.LogExport{}
		Expect(k8sClient.Get(ctx, types.NamespacedName{Namespace: ns, Name: auditExport}, after)).To(Succeed())
