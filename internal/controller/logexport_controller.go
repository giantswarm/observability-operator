...
func (r *LogExportReconciler) reportReady(ctx context.Context, exports []observabilityv1alpha1.LogExport, reason string, renderErr error) error {
	...
	message := "Configuration written for alloy-logexporter"
	if renderErr != nil {
	...
	for i := range exports {
		...
		// Locked: a merge patch replaces the whole conditions array, and these objects
		// come from a cache, so an unlocked write drops conditions added since the list.
