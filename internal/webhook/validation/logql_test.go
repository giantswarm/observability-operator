...
func TestValidateSelector(t *testing.T) {
	tests := []struct {
		...
		// Stream selectors that do not narrow anything.
		{
			name:     "empty stream selector",
			selector: `{}`,
			wantErr:  "needs at least one exact match",
		},
		{
		...
		{
			// An empty value is not an exact match: it matches every stream missing the
			// label, so it narrows nothing.
			name:     "empty value equality",
