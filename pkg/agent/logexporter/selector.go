...
//
// `or` chains and ip() filters are refused rather than guessed at: negating them means
// applying De Morgan across the chain, and a mistranslation here over-exports.
// Validation accepts them today, so a selector using one renders this error until either
// side is brought in line.
...
// filterMatcher unwraps the matcher behind a label filter. Validation has already
// rejected the filterers that carry no matcher.
func filterMatcher(e *syntax.LabelFilterExpr) (*labels.Matcher, error) {
	...
	return nil, fmt.Errorf("label filter %q is not renderable", strings.TrimSpace(e.String()))
}
