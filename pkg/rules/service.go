...
const (
	...
	alloyRulesAppNamespace  = "monitoring"
...
func (s *Service) CleanUp(ctx context.Context, cluster clusterv1.Cluster) error {
	...
	err := controllerutil.RemoveControllerReference(&cluster, configmap, s.Client.Scheme())
	if err != nil {
		logger.Error(err, "failed to remove owner reference from %s", alloyRulesConfigMapName)
		return errors.WithStack(err)
	}
...
func (s Service) createOrUpdateConfigMap(ctx context.Context, cluster clusterv1.Cluster) error {
	...
	_, err = controllerutil.CreateOrUpdate(ctx, s.Client, configmap, func() error {
		...
		err = controllerutil.SetControllerReference(&cluster, configmap, s.Client.Scheme())
		if err != nil {
			logger.Error(err, "failed to set controller reference on %s", alloyRulesConfigMapName)
			return errors.WithStack(err)
		}
