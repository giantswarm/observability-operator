...
func getOrgFromDashboardConfigmap(dashboard *v1.ConfigMap) (string, error) {
	...
	return "", fmt.Errorf("no organization label found in configmap %s/%s", dashboard.Namespace, dashboard.Name)
}

func getDashboardUID(dashboard map[string]interface{}) (string, error) {
	UID, ok := dashboard["uid"].(string)
	if !ok {
		return "", fmt.Errorf("dashboard UID not found in dashboard definition")
	}
	return UID, nil
}
