...
- Auth reconciliation in `ClusterReconciler` now collects all per-signal errors before returning instead of failing fast on the first error, so Mimir/Loki/Tempo auth are all attempted independently.
